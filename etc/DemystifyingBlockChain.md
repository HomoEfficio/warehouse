...
재미있는 것은 블록 체인이 비트코인의 바탕이 되는 체계이지만, 블록 체인이 만들어지고 비트코인이 만들어진 것이 아니라 비트코인을 만들기 위해 고민하던 중에 블록 체인이라는 기술이 탄생했다는 점이다.(위키 등 참고해서 해쉬코인 등 역사얘기를 쓸까말까..)

이제부터 블록 체인이 어떻게 비트코인이라는 화폐의 바탕이 되는 체계가 될 수 있는지 알아보자. 그 전에, 먼저 화폐가 무엇인지 짧게 되새겨 보자.
...
그렇다. 사실 우리는 화폐를 믿고 있다. 화폐의 실체는 바로 믿음이다. 금으로 교환해준다는 보장은 사라졌지만, 화폐로 여전히 치킨을 먹고 맥주를 마실 수 있다고 모두가 믿고 있다. 화폐는 모두의 '신뢰'를 받고 있는 것이다. 비트코인이 가상화폐, 디지털 화폐 또는 암호화 화폐처럼 화폐라는 용어를 달고 있는 이유도 '신뢰'할 만한 대상이 될 수 있기 때문이다. 그 신뢰의 원천이 바로 블록 체인이다.
...
자 이제부터 진짜 블록 체인의 내부로 들어가볼 차례다. 블록 체인이라는 체인을 이루는 원소인 블록은 무엇일까?
...
개별 거래 정보는 결국 머클 트리의 해쉬값인 `merklehash` 값으로 집약된다. 블록 헤더의 6가지 정보 중에서 `version`, `previousblockhash`, `merklehash`, `time`, `bits` 이렇게 5가지는 블록 해쉬를 만드는 시점에서 이미 확정되어 변하지 않는 값이다. 하지만 마지막 `nonce`는 확정되어 있지 않고 새로 구해야 하는 값이다. 이 `nonce` 값을 구해서 최종적으로 블록 해쉬 값을 구하고, 이 블록 해쉬값을 식별자로 가지는 유효한 블록을 만들어내는 것이 바로 작업 증명(Proof of Work), 흔히 말하는 채굴이다.
...
어디서 많이 본 구조 아닌가? 그렇다. 블록 체인은 사실 링크드 리스트다.
...
앞에서 `nonce`값을 구해서 최종적으로 블록 해쉬값을 구하고, 이 블록 해쉬값을 식별자로 가지는 유효한 블록을 만들어내는 것이 작업 증명이라고 했다. 결국 `nonce` 값을 구하는 것이 핵심이다. 

### nonce와 작업 증명

`nonce`값은 구하는 방법을 알아보기 전에 먼저 `nonce`값이란 어떤 값을 의미하는 지 알아야 한다.
...
그렇다면 작업 증명 성공 여부의 기준이 되는  `000000a84...`라는 값은 어떻게 정해지나? 그 값은 작업 난이도에 의해 결정되는데, 이 부분은 핵심은 아니므로 마지막 부분의 FAQ를 참고하자.

### 보상

보상은 **새로 발행되는 비트코인과 해당 블록에 포함되는 거래의 거래 수수료의 합**이다. 비트코인의 새로운 발행은, 채굴자가 블록을 처음 구성할 때 채굴자의 지갑으로 일정량의 비트코인이 입금되는 거래를 그 블록의 첫 거래(generation transaction)로 추가하는 방식으로 이루어진다. 새로 발행되는 비트코인은 최초에 50BTC에서 시작해서 블록 체인에 21만개의 블록이 추가될 때마다 절반으로 줄어들어 2017년 7월 현재는 블록 당 12.5BTC가 새로 발행되며, 보상의 일부로서 채굴자에게 주어진다.
...
## 블록 체인의 두번째 핵심 - 합의 알고리듬

블록 체인의 첫번째 핵심은 작업 증명이며, 이는 블록 단위에서 처리되는 일이다. 블록 체인의 두번째 핵심은 합의 알고리듬인데, 이는 블록 단위가 아니라 블록 체인 단위에서 처리되는 일이다. 합의 알고리듬이 무엇인지 또 왜 필요한지 알려면 블록 체인을 분산 환경에서 바라볼 필요가 있다.
...
하나의 거래 정보가 발생하면, 거래 정보를 받은 노드는 먼저 해당 거래의 유효성을 검증한 후에 인접한 다른 노드에 거래 정보를 전파하고, 그 거래를 아직 블록 생성 작업이 시작되지 않은 후보 블록에 추가한다. 거래 정보의 전파를 그림으로 나타내면 다음과 같다.

%%% 거래 정보 전파 그림

### 블록의 생성 및 전파

블록에 거래 정보가 채워지면 노드는 블록을 생성한다. 이 때 칠레에 있던 노드와 한국에 있던 노드는 상당히 멀리 떨어져 있으므로, 각 후보 블록에 담겨 있는 거래의 내용과 순서는 아래와 같이 서로 다를 수 있다. 

%%% 칠레, 한국 블록 내의 거래 내용 차이

새로 만들어질 블록은 아래 그림과 같이 동일한 %%%ABC 블록의 다음에 추가될 예정이다.

%%% %%%ABC 까지의 블록 그림

이 때 칠레의 노드와 한국의 노드가 거의 동시에 `nonce` 값을 찾아서 블록을 성공적으로 생성했다. 한국의 노드가 생성한 블록을 %%%한국이라 하고 칠레의 노드가 생성한 블록을 %%%칠레라고 하면, 한국의 노드는 %%%ABC에 %%%한국 블록을 추가하고 인접노드에 %%%한국 블록을 전파하고, 칠레의 노드는 %%%ABC에 %%%칠레 블록을 추가하고 인접 노드에 %%%칠레 블록을 전파한다.

%%% 한국 노드의 블록 체인 그림
%%% 칠레 노드의 블록 체인 그림

한국 노드의 인접 노드에서는 전달받은 %%%한국 블록의 블록 해쉬를 다시 계산해서 그 값이 정말로 특정한 숫자보다 작은지 검증한 후에, 자신이 가지고 있던 %%%ABC 노드에 %%%한국 블록을 추가한다. 칠레 노드에 인접한 노드에도 마찬가지 방식으로 %%%칠레 노드가 추가된다. 이 방식이 계속되면서 %%%한국 블록과 %%%칠레 블록은 전 세계에 분산되어 있는 노드에 전파된다.
...
한편 터키에 있는 노드도 %%%ABC 까지 가지고 있었고 나름 `nonce`값을 구하고 있었으나, 구하기 전에 아쉽게도 %%%한국 블록을 먼저 전달받아서 %%한국 블록을 검증한 후 %%%ABC 블록 다음에 추가했다. 그런데 바로 직후에 %%%칠레 블록을 전달 받았다. 이 두 블록의 바로 앞 블록은 모두 %%%ABC다. 따라서 터키 노드의 블록 체인은 다음과 같이 분기가 발생한다.

%%% 터키 노드의 블록 체인의 분기

%%%한국 블록이 붙어있는 블록체인과 %%%칠레 블록이 붙어있는 블록체인은 내용이 서로 다르다. 블록 체인은 신뢰의 원천이 되는 체계라고 했는데, 이렇게 정보가 갈라지는 체계를 믿을 수 있는걸까?

### 어느 블록 체인을 믿을 것 인가?

블록 체인에서는 이렇게 분기가 발생했을 때 어느 블록 체인을 믿을 것인가에 대한 기준이 합의가 되어 있어야 한다. 그렇지 않으면 분산되어 있는 장부 내용의 동기화가 깨지기 때문이다. 그래서 분기된 블록 체인 선택에 대한 합의 기준을 합의 알고리듬이라고 한다. 

>블록 체인의 합의 알고리듬은 길이가 더 긴 블록 체인을, 다시 말하면, 더 많은 작업 증명이 수행된 블록 체인을 선택하게 되어 있다.

터키 노드에서 블록 체인이 두 개로 갈라진 순간에서는 어느 한 쪽을 선택할 수 없다. 왜냐하면 분기된 두 블록 체인의 길이가 같기 때문이다. 하지만 이 상태는 오래 가지 않고 곧 해결 된다. 블록 생성은 평균 10분이 소요될 정도로 연산량이 큰 작업이며, 따라서 앞에서 발생한 것처럼 %%%한국 블록과 %%%칠레 블록이 거의 동시에 생성되어 분기가 발생할 가능성은 그리 높지 않기 때문이다. 그래서 결국에는 %%%한국 블록을 기준으로 분기된 블록 체인의 길이와 %%%칠레 블록을 기준으로 분기된 블록 체인의 길이가 달라지는 순간이 금방 다가오기 때문이다.

%%% 길이가 달라져서 한 쪽이 선택되는 순간

위 그림처럼 터키 노드에서 두 블록 체인의 길이가 달라지는 순간, 더 긴 쪽을 선택한다는 합의 알고리듬에 의해 둘 중 어느 한 쪽이 믿을 만한 블록 체인으로 선택되며, 나머지 한 쪽은 버려진다. 이는 터키 노드 뿐아니라 일시적으로 분기되어 있던 블록 체인을 가지고 있던 모든 노드에서도 동일한 방식으로 전개된다.

하지만 일시적으로나마 이런 분기 상태가 발생할 수 있기 때문에, 실제 거래 상황에서는 보통 어떤 거래가 포함된 블록 이후로 3~5개의 블록이 더 추가된 후에  그 거래를 최종적으로 유효한 것으로 확정한다. 일시적으로 분기가 발생하더라도 그 이후로 3~5개의 블록이 추가되는 과정에서 어느 하나의 블록 체인만 남기 때문이다.
...
블록 하나 채굴하는데 평균 10분이 소요되고, http://bitcoincharts.com/bitcoin/ 에서 확인한 $$$2017년 기준으로 변경 필요 - 2016-01-23 현재 총 블록 수가 약 40만 블록이므로, 최초의 원조 블록인 Genesis 블록에 포함된 거래를 변경하면 약 400만 분, 약 7.6년의 시간이 소요된다.
...
>
>이는 많은 시간이 소요되고, 그동안에도 변경 되지 않은 원래의 블록 체인에는 다른 노드들에 의해 블록이 계속 추가되어 A보다 길이가 길어지게 되므로,
>
>완료된 거래 정보의 변경은 현실적으로 사실상 불가능하다. 



## 마무리

지금까지 블록 체인이 어떻게 비트 코인이라는 암호화 화폐의 신뢰의 원천이 될 수 있는가라는 물음에 대한 답을 찾아봤다.

블록 체인은 거대한 분산 공개 장부이며, 그 장부 안에 포함된 개별 거래는 모두 디지털 서명이 붙어 있어서 은행이나 다른 제 3자의 개입이 없어도 진본임을 보증할 수 있다. 또 수천, 수만노드에 분산 되어 있어서 어느 한 지점에 장애나 공격이 발생하더라도 블록 체인이라는 네트워크 전체는 문제 없이 계속 돌아갈 수 있다.

그리고 블록 체인은 작업 증명이라는 수학적 계산 작업과 합의 알고리듬이라는 동기화 로직을 통해 위/변조가 사실상 불가능한 구조를 갖게 되어, 그 안에 기록된 거래들은 은행같은 중앙의 보증 기관이 없이도 신뢰할 수 있는 거래로서 확정될 수 있다.

블록 체인은 거래 당사자간의 신뢰 확보를 위해 중앙 기관을 필요로 하지 않는 탈중앙화(Decentralization)를 달성한 최초의 소프트웨어 기술이다. 블록 체인이 세상을 바꿀 수 있는 기술이라고 평가받는 이유도 바로 이 탈중앙화에 있다.
...
비트코인 지갑은 송수금 거래를 가능하게 해주는 클라이언트 소프트웨어이며, 거래 정보를 블록 체인 네트워크에 전파해야 하므로 블록 체인 네트워크의 노드이기도 하다. 하지만, 지갑은 작업 증명 계산을 하지 않기 때문에 블록 체인의 모든 거래 정보를 저장할 필요가 없고, 저장하지도 않는다. 블록 체인 네트워크에 참여하는 노드는 모든 기능을 가진 비트코인 코어,  
