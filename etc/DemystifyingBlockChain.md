...
제목은 블록 체인 이야기라고 써놓고, 서두에서는 비트코인 얘기만 했다. 비트코인과 블록 체인이 무엇인지 구별하는 것이 좋겠다는 의도에서 일부러 그렇게 기술했다.
...
채굴에 대해 알아보기 전에, 이제 블록이 무엇인지 알게 되었으니 블록이 이어져서 만들어지는 블록 체인을 기술적으로 다시 정의해보자.

## 블록 체인이란?

블록 체인은 앞에서는 거대한 분산 공개 장부라고 했는데, 그것은 개념적인 정의이고, 기술적으로는 블록이 이어져서 만들어진 블록의 집합체라고 말할 수 있다.
...
블록 체인의 첫번째 핵심은 작업 증명이며, 이는 블록 단위에서 처리되는 일이다. 블록 체인의 두번째 핵심은 블록 단위가 아니라 블록 체인 단위에서 처리되는 일이다. 앞에서 블록 체인은 거대한 분산 공개 장부라고 했다. 두번째 핵심을 알아보기 위해서는 블록 체인을 분산 환경에서 바라볼 필요가 있다.

$$$ 칠레에서의 블록 생성과 한국에서의 블록 생성 그림
...
### 블록을 생성하는데 성공한 한 명의 채굴자만 보상을 받는건가?

### 완료된 거래 정보의 변경이 사실 상 불가능한 이유
